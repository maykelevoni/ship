...
export async function sendEmail(
  ...
  const senderEmail =
    params.fromEmail ?? (await getSetting("brevo_sender_email", userId));
  if (!senderEmail) {
  ...
  const senderName =
    params.fromName ??
    (await getSetting("brevo_sender_name", userId)) ??
    "PostForge";

  const toEmail =
    params.toEmail ?? (await getSetting("brevo_to_email", userId));
  if (!toEmail) {
