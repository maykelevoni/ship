...
export async function analyzeOpportunities(userId: string): Promise<void> {
  ...
  if (topics.length === 0) {
    console.log(
      "[opportunities] No research topics found for today — skipping analysis",
    );
    return;
  }

  console.log(
    `[opportunities] Found ${topics.length} research topic(s) for today`,
  );
  ...
  if (existing > 0) {
    console.log(
      `[opportunities] ${existing} PromotionOpportunity row(s) already exist for today — skipping`,
    );
    return;
  }
  ...
  try {
    ...
    throw new Error(
      `[opportunities] Failed to parse AI response as JSON: ${detail}\nRaw response: ${result.text}`,
    );
  }
  ...
  // 9. Log count saved
  console.log(
    `[opportunities] Saved ${data.length} PromotionOpportunity row(s) for today (provider: ${result.provider})`,
  );
}
