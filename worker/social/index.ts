...
export async function runSocialRepurposing(): Promise<void> {
  ...
  if (!blogPost) {
    console.log('[social] No blog post for today — skipping social repurposing')
    return
  }

  console.log(`[social] Repurposing blog post: "${blogPost.title}" (${blogPost.id})`)

  // 3. Load templates from DB
  const templateRows = await db.template.findMany({
    where: { platform: { in: [...PLATFORMS] }, active: true },
  })
  const templates = Object.fromEntries(templateRows.map((t) => [t.platform, t]))
  ...
  const savedPieces: Partial<Record<SocialPlatform, { id: string; content: string }>> = {}
  ...
  for (let i = 0; i < PLATFORMS.length; i++) {
    ...
    if (result.status === 'rejected') {
      console.error(`[social] Generation failed for "${platform}":`, result.reason)
      continue
    }
    ...
    try {
      ...
      console.log(`[social] Saved "${platform}" piece (id: ${piece.id}, provider: ${provider})`)
    } catch (err) {
