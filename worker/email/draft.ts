...
export async function generateEmailDraft(userId: string): Promise<void> {
  ...
  if (!blogPost) {
    console.log(
      "[email/draft] No blog post found for today — skipping email draft generation",
    );
    return;
  }
  ...
  if (existing) {
    console.log(
      `[email/draft] EmailDraft already exists for blogPostId ${blogPost.id} — skipping`,
    );
    return;
  }
  ...
  // 9. Log success
  console.log(
    `[email/draft] EmailDraft saved for blogPostId ${blogPost.id} (subject: "${subject}")`,
  );
}
