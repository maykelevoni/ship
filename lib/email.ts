...
export const sendVerificationRequest: EmailConfig["sendVerificationRequest"] =
  async ({ identifier, url, provider }) => {
    ...
    const authSubject = isNewUser || !userVerified
      ? "Activate your account"
      : `Sign-in link for ${siteConfig.name}`;
    const greeting = user?.name ? `Hi ${user.name},` : "Hi there,";
    const bodyText = isNewUser || !userVerified
  ...
  <div style="max-width:600px;margin:0 auto;padding:40px 20px;">
    ...
    <p style="margin:24px 0;">
      <a href="${url}" style="display:inline-block;background:#18181b;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-size:15px;">
        ${isNewUser || !userVerified ? "Activate Account" : "Sign in"}
      </a>
    </p>
    <p style="font-size:14px;color:#666;">This link expires in 24 hours and can only be used once.</p>
    ${!isNewUser && userVerified ? '<p style="font-size:14px;color:#666;">If you did not try to log in, you can safely ignore this email.</p>' : ""}
  </div>
</body>
</html>`;

    // DEV: log the magic link so we can test without a real inbox
