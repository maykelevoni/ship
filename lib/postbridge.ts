...
async function getHeaders(userId: string): Promise<HeadersInit> {
...
export async function schedulePost(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `post-bridge schedulePost failed (${response.status}): ${text}`,
    );
  }
...
export async function getConnectedAccounts(userId: string): Promise<Account[]> {
  const headers = await getHeaders(userId);
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(
      `post-bridge getConnectedAccounts failed (${response.status}): ${text}`,
    );
  }
