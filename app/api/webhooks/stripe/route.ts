...
// Required so Next.js does not parse the body — Stripe needs the raw bytes for signature verification
export const config = {
  api: {
    bodyParser: false,
  },
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!stripeSecretKey || !webhookSecret) {
      console.warn('[stripe webhook] Missing stripe_secret_key or stripe_webhook_secret in settings')
