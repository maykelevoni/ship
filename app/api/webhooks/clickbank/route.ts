...
export async function POST(req: NextRequest) {
  try {
    ...
    const secretKey = process.env.CLICKBANK_SECRET;
    if (secretKey) {
      const signature = req.headers.get("x-clickbank-signature");
      if (!signature) {
        console.warn(
          "[clickbank webhook] Missing x-clickbank-signature header; skipping verification",
        );
      } else {
        ...
        if (signature !== expected) {
          console.warn(
            "[clickbank webhook] Signature mismatch; ignoring event",
          );
