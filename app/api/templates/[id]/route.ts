...
export const PATCH = auth(
  async (req, { params }: { params: { id: string } }) => {
    ...
    try {
      ...
      const existing = await db.template.findFirst({ where: { id, userId } });
      if (!existing) {
      ...
      if (
        ...
        !VALID_PLATFORMS.includes(
          data.platform as (typeof VALID_PLATFORMS)[number],
        )
      ) {
    ...
    try {
      ...
      if (usageCount > 0) {
        return Response.json(
          {
            error:
              "Template is used in schedule. Remove schedule entries first.",
          },
          { status: 409 },
        );
      }
